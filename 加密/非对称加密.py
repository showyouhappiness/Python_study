...
数据库 将password和name组合之后进行加密，然后每次登陆的时候比对加密后的字符串
...
with open("public_a.rsa") as f:
    ...
    print(rsa_text, '使用公钥对用户密码进行RSA加密')
...
with open("private_a.rsa") as f:
    ...
    print(signature.decode('utf-8'), '使用私钥生成签名')
# 使用公钥验证签名
with open("public_a.rsa") as f:
    key = f.read()
    ...
    print(verifier.verify(digest, base64.b64decode(signature)), '使用公钥验证签名')
